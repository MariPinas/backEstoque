...
export class ProdutoRepository{
    ...
    async insertProduto(produto: Produto): Promise<Produto> {
        const query = "INSERT INTO estoque.Produto (nome, preco, descricao, imagem, quantidade, usuario_id) VALUES (?, ?, ?, ?, ?, ?)" ;
    
        try {
            const resultado = await executarComandoSQL(query, [produto.nome, produto.preco, produto.descricao, produto.imagem, produto.quantidade, produto.usuario_id]);
    ...
    async updateProduto(produto: Produto): Promise<void> {
        const query = "UPDATE estoque.Produto SET nome = ?, preco = ?, descricao = ?, imagem = ?, quantidade = ? WHERE id = ? AND usuario_id = ?";
    
        try {
            const resultado = await executarComandoSQL(query, [produto.nome, produto.preco, produto.descricao, produto.imagem, produto.quantidade, produto.id, produto.usuario_id]);
            if (resultado.affectedRows === 0) {
                throw new Error('Nenhum produto encontrado ou o usuário não tem permissão para atualizar este produto');
            }
    ...
    async deleteProduto(produto:Produto) :Promise<Produto>{
        ...
        try {
            ...
            if (resultado.affectedRows === 0) {
                throw new Error('Nenhum produto encontrado ou o usuário não tem permissão para deletar este produto');
            }
    ...
    async filterProduto(id: number, usuario_id:number) :Promise<Produto>{
        ...
        try {
            ...
            if (resultado.affectedRows === 0) {
                throw new Error('Nenhum produto encontrado ou o usuário não tem permissão para ver este produto');
            }
